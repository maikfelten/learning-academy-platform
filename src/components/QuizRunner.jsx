...
function Lauf({ versuch, onAbgeben, laeuft }) {
  ...
  return (
    <div className="panel-flat space-y-5 p-5">
      ...
      {warnung && (
        <p className="text-[12px]" style={{ color: 'var(--status-soon-text)' }}>
          {warnung}
        </p>
      )}
    ...
    <div className="space-y-4">
      <div className="panel-flat flex flex-col items-center gap-4 p-6 text-center">
        <div
          ...
          style={{ background: `conic-gradient(${farbe} ${ergebnis.prozent * 3.6}deg, var(--tint-3) 0)` }}
        >
      ...
      {/* Themen der falschen Fragen - ohne Lösungen */}
      ...
      {/* Volle Auflösung - nach Bestehen (oder wenn im Quiz so eingestellt) */}
      {ergebnis.aufloesung && (
        <div className="panel-flat p-5">
